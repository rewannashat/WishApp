...
*/
  ...
  Future<void> toggleFavorite(Map<String, String?> stream) async {
    final prefs = await SharedPreferences.getInstance();

    final exists = favoriteLiveList.any((item) => item['title'] == stream['name']);

    if (exists) {
      favoriteLiveList.removeWhere((item) => item['title'] == stream['name']);
    } else {
      favoriteLiveList.add({
        'title': stream['name'] ?? '',
    ...
    final encoded = favoriteLiveList.map((e) => jsonEncode(e)).toList();
  ...
  bool isFavorite(String title) {
    return favoriteLiveList.any((item) => item['title'] == title);
  }
...
// Load recent views from SharedPreferences
  ...
  Future<void> addToRecent(Map<String, String> series) async {
    ...
    recentLiveList.removeWhere((item) => item['title'] == updatedSeries['title']);
